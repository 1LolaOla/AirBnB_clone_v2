...
class test_basemodel(unittest.TestCase):
    ...
    def test_todict(self):
        ...
        mdl = self.value()
        mdl.firstname = 'Tehila'
        mdl.lastname = 'Ola'
        self.assertIn('firstname', mdl.to_dict())
        self.assertIn('lastname', mdl.to_dict())
        self.assertIn('firstname', self.value(firstname='Tehila').to_dict())
        self.assertIn('lastname', self.value(lastname='Ola').to_dict())
        ...
        if os.getenv('HBNB_TYPE_STORAGE') != 'db':
            self.assertDictEqual(
                self.value(id='u-b34', age=45).to_dict(),
                {
                    '__class__': mdl.__class__.__name__,
                    'id': 'u-b34',
                    'age': 45
